...
@register_attack("loss")
class LossAttack:
    """Loss 黑盒基线攻击（Yeom et al., 2018）。

    针对遗忘模型：遗忘集成员由于遗忘不彻底而具有更低的逐样本损失，
    测试集非成员从未被训练、损失偏高。membership score = -loss。
    """
    ...
    def _budget(cls, context: AttackContext) -> int:
        return int(context.attack_options.get("loss_sample_budget", 500))
    ...
    def run(cls, context: AttackContext, dry_run: bool = False) -> AttackResult | None:
        if dry_run:
            print(f"[attack:loss] dry-run {context.experiment_name} seed {context.seed}")
            return None
        ...
        for is_member, indices, src in ((True, member_idx, train_det), (False, nonmember_idx, test_det)):
            ...
            for images, _, targets in loader:
                ...
                # 遗忘集成员 loss 更低，故 membership score 取负 loss
                scores.extend((-loss).detach().cpu().numpy().tolist())
        ...
        result = AttackResult(
            ...
            raw={
                "signal": "per-sample cross-entropy loss (negated)",
                "n_member": n_member,
                "n_nonmember": n_nonmember,
            },
        )

        summary_path = Path(context.attack_result_dir("loss")) / "attack_result.json"
